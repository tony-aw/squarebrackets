...
sb_test <- function(x, ...)i_mod(x, ..., rp = x[1])
...
expect_error(
  ss_mod(x, list(1:2, 1:2), c(1,3), rp = 1:6),
  pattern = "recycling not allowed"
)
expect_error(
  ss_mod(x, list(1:2, 1:2), c(2,3), rp = 1:6),
  pattern = "recycling not allowed"
)
