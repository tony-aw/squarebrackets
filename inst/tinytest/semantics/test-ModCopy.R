...
for(i in seq_along(convertfuns)) {
  ...
  z <- x
  tt_mod(z, row = rows, col = cols, tf = \(x)x[1])
...
i <- c(1, 3)
z <- x
ii_mod(z, i = i, tf = \(x)x[1])

az <- sapply(ii_x(z, i = i), data.table::address)
ax <- sapply(ii_x(x, i = i), data.table::address)
ay <- sapply(ii_x(y, i = i), data.table::address)
