...
for(iV in v) {
  for(iInv in rel) {
    for(i in seq_along(from)) {
      expected[[counter]] <- tempfun1(x, iV, iInv, start[i], end[i])
      out[[counter]] <- countv(x, v = iV, na = TRUE, r = iInv, from = from[[i]], to = to[[i]])
      counter <- counter + 1L
    }
  }
}
...
for(iV in seq_along(v)) {
  for(iInv in rel) {
    for(i in seq_along(from)) {
      ...
      out[[counter]] <- countv(x, v = v[[iV]], na = TRUE, r = iInv, from = from[[i]], to = to[[i]])
      counter <- counter + 1L
    }
  }
}
...
for(iV in seq_along(v)) {
  for(iInv in rel) {
    for(i in seq_along(from)) {
      expected[[counter]] <- tempfun2(x, v[[iV]], iInv, start[i], end[i])
      out[[counter]] <- countv(x, v = v[[iV]], na = TRUE, r = iInv, from = from[[i]], to = to[[i]])
      counter <- counter + 1L
    }
  }
...
for(iV in seq_along(v)) {
  for(iInv in rel) {
    for(i in seq_along(from)) {
      expected[[counter]] <- tempfun_multistring(x, v, iInv, start[i], end[i])
      out[[counter]] <- countv(x, v = v, na = TRUE, r = iInv, from = from[[i]], to = to[[i]])
      counter <- counter + 1L
    }
  }
}
...
for(iV in seq_along(v)) {
  for(iInv in rel) {
    for(i in seq_along(from)) {
      expected[[counter]] <- tempfun1(unclass(x), v[[iV]], iInv, start[i], end[i])
      out[[counter]] <- countv(x, v = v[[iV]], na = TRUE, r = iInv, from = from[[i]], to = to[[i]])
      counter <- counter + 1L
    }
  }
