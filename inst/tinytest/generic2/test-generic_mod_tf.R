...
f_out.data.frame <- function(x, s = NULL, d = NULL, obs = NULL, vars = NULL) {
  ...
  return(ss2_mod.data.frame(x, s, d, obs, vars, tf = \(x)x[1]))
...
# test errors ====
sb_test <- function(...)ii2_mod(..., tf = \(x)x[1])
...
expect_error(
  ii2_mod(as.list(1:10), i = 1, tf = "foo"),
