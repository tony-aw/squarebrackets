...
x <- as.list(1:10)
expect_equal(
  sb2_wo(x, 1:9, red = TRUE),
  x[[10]]
)
expect_equal(
  sb2_wo(x, c(FALSE, rep(TRUE, 9)), red = TRUE),
  x[[1]]
)
...
f_out.data.frame <- sb2_wo.data.frame
