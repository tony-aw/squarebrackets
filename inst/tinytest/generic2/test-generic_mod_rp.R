...
temp.fun.matrix <- function(x, row, col) {
  for(i in 1:length(row)) {
    for(j in 1:length(col)) {
      ...
      expect_equal(
        sb2_mod(x, row = row[[i]], col = col[[j]], rp = rp),
        subset_mat(x, row[[i]], col[[j]], rp = rp)
      ) |> errorfun()
      expect_true(sb2_mod(x, row = row[[i]], col = col[[j]], rp = rp) |>
                    is.matrix()) |> errorfun()
...
temp.fun.1d <- function(x, row) {
  for(i in 1:length(row)) {
    rp <- as.list(seq_along(indx_x(row[[i]], x, dimnames(x)[[1]], length(x))))
    expect_equal(
      sb2_mod(x, row[[i]], 1, rp = rp),
      subset_1d(x, row[[i]], rp = rp)
    ) |> errorfun()
    expect_true(sb2_mod(x, row[[i]], 1, rp = rp) |>
                  is.array()) |> errorfun()
    assign("enumerate", enumerate + 2, envir = parent.frame(n = 1))
  }
}
...
temp.fun.2d <- function(x, row, col) {
  for(i in 1:length(row)) {
    for(j in 1:length(col)) {
      ...
      expect_equal(
        sb2_mod.array(x, sub, dims, rp = rp),
        subset_mat(x, row[[i]], col[[j]], rp = rp)
      ) |> errorfun()
      expect_true(sb2_mod.array(x, sub, dims, rp = rp) |>
                    is.array()) |> errorfun()
      assign("enumerate", enumerate + 2, envir = parent.frame(n = 1))
    }
  }
      ...
      for(k in 1:length(filter)) {
        for(l in 1:length(get_vars)) {
          ...
          if(!wrong1 && !wrong2 && !wrong3) {
            ...
            expect_equivalent(
              sb2_mod(x, row[[i]], col[[j]], filter[[k]], get_vars[[l]], rp = rp),
              mysubset(x, row[[i]], col[[j]], filter[[k]], get_vars[[l]])
            ) |> errorfun()
            expect_true(
              sb2_mod(x, row[[i]], col[[j]], filter[[k]], get_vars[[l]], rp = rp) |> is.data.frame()
            )
            assign("enumerate", enumerate + 2, envir = parent.frame(n = 1))
          }
        }
...
expect_error(
  sb2_mod(x, list(1:2, 1:2), c(1,3), rp = as.list(1:6)),
  pattern = "recycling not allowed"
)
expect_error(
  sb2_mod(x, list(1:2, 1:2), c(2,3), rp = as.list(1:6)),
  pattern = "recycling not allowed"
)
