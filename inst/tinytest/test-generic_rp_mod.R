      ...
      for(k in 1:length(filter)) {
        for(l in 1:length(get_vars)) {
          ...
          if(!wrong1 && !wrong2 && !wrong3) {
            ...
            expect_equivalent(
              sb_mod(x, row[[i]], col[[j]], filter[[k]], get_vars[[l]], rp = rp),
              mysubset(x, row[[i]], col[[j]], filter[[k]], get_vars[[l]])
            ) |> errorfun()
            expect_true(
              sb_mod(x, row[[i]], col[[j]], filter[[k]], get_vars[[l]], rp = rp) |> is.data.frame()
            )
            assign("enumerate", enumerate + 2, envir = parent.frame(n = 1))
          }
        }
...
x <- as.list(1:10)
expect_error(
  sb_mod(x, i = 1:5, rp = 1:10),
  pattern = "`rp` must be a list"
)
expect_error(
  sb_mod(x, i = 1:5, rp = as.list(1:6)),
  pattern = "recycling not allowed"
)
enumerate <- enumerate + 3
