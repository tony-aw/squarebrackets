...
for(i in seq_along(convertfuns)) {
  ...
  z <- sb2_mod(x, vars = cols, tf = \(x)x[1])
  
  az <- sapply(sb2_x(z, vars = cols), data.table::address)
  ax <- sapply(sb2_x(x, vars = cols), data.table::address)
  ay <- sapply(sb2_x(y, vars = cols), data.table::address)
  expect_false(any(ax == az)) |> errorfun()
  expect_false(any(ay == az)) |> errorfun()
  expect_true(all(ax == ay)) |> errorfun()
  
  az <- sb2_x(z, vars = cols)
  ax <- sb2_x(x, vars = cols)
  ay <- sb2_x(y, vars = cols)
  expect_false(identical(ax, az)) |> errorfun()
  expect_false(identical(ay, az)) |> errorfun()
  expect_true(identical(ax, ay)) |> errorfun()
  
  az <- sapply(sb2_wo(z, vars = cols), data.table::address)
  ax <- sapply(sb2_wo(x, vars = cols), data.table::address)
  ay <- sapply(sb2_wo(y, vars = cols), data.table::address)
  expect_true(all(ax == az)) |> errorfun()
  expect_true(all(ay == az)) |> errorfun()
  expect_true(all(ax == ay)) |> errorfun()
  
  az <- sb2_wo(z, vars = cols)
  ax <- sb2_wo(x, vars = cols)
  ay <- sb2_wo(y, vars = cols)
...
for(i in seq_along(convertfuns)) {
  ...
  z <- sb2_mod(x, obs = rows, vars = cols, tf = \(x)x[1])
...
az <- sapply(sb2_wo(z, i = i), data.table::address)
ax <- sapply(sb2_wo(x, i = i), data.table::address)
ay <- sapply(sb2_wo(y, i = i), data.table::address)
