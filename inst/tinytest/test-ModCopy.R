...
for(i in seq_along(convertfuns)) {
  ...
  az <- sapply(sb2_wo(z, col = cols), data.table::address)
  ax <- sapply(sb2_wo(x, col = cols), data.table::address)
  ay <- sapply(sb2_wo(y, col = cols), data.table::address)
  expect_true(all(ax == az)) |> errorfun()
  expect_true(all(ay == az)) |> errorfun()
  expect_true(all(ax == ay)) |> errorfun()
  
  az <- sb2_wo(z, col = cols)
  ax <- sb2_wo(x, col = cols)
  ay <- sb2_wo(y, col = cols)
