...
expect_error(
  stride_v(~ form, v = 1L),
  pattern = "couldb.mutatomic(y) is not TRUE",
  fixed = TRUE
)
expect_error(
  stride_v(integer(0L), v= 1L),
  pattern = "zero-length `y` not allowed"
)
expect_error(
  stride_v(1, na = 0.5),
  pattern = "`na` must be a logical scalar"
)
expect_error(
  stride_v(1, na = c(TRUE, FALSE)),
  pattern = "`na` must be a logical scalar"
)
