...
for(iF in seq_along(list_fromto)) {
  for(iT in seq_along(list_fromto)) {
    for(iB in seq_along(list_by)) {
      ...
      to = list_fromto[[iT]]
      by = list_by[[iB]]
      expected[[counter]] <- tempfun1(x, from, to, by, 1L)
      out[[counter]] <- tempfun2(x, from, to, by, 1L)
      counter <- counter + 1L
      enumerate <- enumerate + 1L
    }
  }
