...
fi_x(x, "a") # selects all indices with the name "a"
...
fi_x(x, 1:3 * -1i) # select last 3 indices
fi_x(x, 3:1 * -1i) # select last 3 indices in tail()-like order
...
'squarebrackets' provides a (somewhat) consistent syntax to invert indices:
 ...
 - The methods whose names end with `_x` (like the `fi_x()` shown before) perform extraction;  
...
fi_x(x, "a") # extract element "a" with 'squarebrackets'
fi_wo(x, "a") # remove element "a" with 'squarebrackets'; same syntax
...
For the `A` part, the following is available:
 ...
 - `fi_`: operates on subsets of atomic objects by (flat/linear) indices.
 - `fi2_`: operates on subsets of recursive objects by (flat/linear) indices.
...
When `y` is a list (i.e. recursive), the following holds (roughly speaking):
 ...
 - `fi2_x(y, i)` corresponds to `y[i]` or `y[[i]]` (depending on the arguments given in `fi2_x()`)
