...
For the `B` part, the following is available:
 ...
 - `⁠_`x⁠: extract, exchange, or duplicate (if applicable) subsets.
 - `⁠_w`o⁠: returns the original object without the provided subsets.
 - `⁠_mo`d⁠: modify subsets and return copy.
...
The `s` argument must be a list or atomic vector, specifying the subscripts (i.e. dimensional indices).  
...
I.e. :
 ...
 - To specify columns `2:5`, use `s = 2:5` and `d = 2`.
 - To specify layers (the third dimension)  `4:9`, use `s = 4:9` and `d = 3`.
 - To specify rows `1:10` **and** columns `2:5`, use `s = list(1:10, 2:5)` and `d = 1:2`.
 - To specify both rows and columbs `1:5`, one can use `s = 1:5` and `d = 1:2`.  
 Notice you don't have to write `n(1:5, 1:5)` or `rep(n(1:5), 2)`:  
 if `s` is an atomic vector or a list of length 1, 'squarebrackets' will use the same indices from  `s` to all dimensions specified in `d`.

The `d` argument has the default specification `1:ndim(x)`, where `ndim(x) = length(dim(x))`.  
So to specify rows `1:5` and columns `1:4` for a matrix (which by definition has 2 dimensions), specifying `s = n(1:5, 1:4)` is enough, and you don't even have to specify the `d` argument.  
...
Note that `n()` is short-hand for `list()`; `n()` **nests** multiple objects together, just like `c()` concatenates multiple objects together.
