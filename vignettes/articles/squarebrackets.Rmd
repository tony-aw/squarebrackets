...
ii_x(x, 1:3 * -1i) # select last 3 indices
ii_x(x, 3:1 * -1i) # select last 3 indices in tail()-like order
...
For the `A` part, the following is available:
 ...
 - `ii_`: operates on subsets of atomic objects by (flat/linear) indices.
 - `ii2_`: operates on subsets of recursive objects by (flat/linear) indices.
...
When `y` is a list (i.e. recursive), the following holds (roughly speaking):
 ...
 - `ii2_x(y, i)` corresponds to `y[i]` or `y[[i]]` (depending on the arguments given in `ii2_x()`)
