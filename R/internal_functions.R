...
.internal_abind <- function(
    arg.list, along, atomic, name_along, name_flat
) {
  N <- max(
    1L,
  ...
  if(atomic) {
    out <- array(
      unlist(arg.list, use.names = FALSE),
  ...
  if(name_flat) {
    arg.flatnames <- lapply(arg.list, names)
    arg.lens <- collapse::vlengths(arg.list, use.names = FALSE)
    flatnames <- array(
      .bind_make_flatnames(out, arg.list, arg.flatnames, arg.lens),
      dim = c(arg.dim[-along, 1L], sum(arg.dim[along, ]))
    )
  }
  ...
  if (any(order(perm) != seq_along(perm))) {
    ...
    if(name_flat) {
      flatnames <- aperm.default(flatnames, order(perm))
      names(out) <- flatnames
    }
  }
