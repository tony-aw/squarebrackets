...
.abind <- function (..., ._along) 
{
  ...
  if (is.character(new.names)) {
    arg.names[seq_along(new.names)[nchar(new.names) > 0]] <- new.names[nchar(new.names) > 
                                                                           0]
    new.names <- NULL
  }
  if (collapse::anyv(arg.names, "")) {
    if (make.names) {
      ...
      arg.alt.names <- arg.names
      arg.alt.names[arg.names == ""] <- paste("X", seq_along(arg.names), 
                                              sep = "")[arg.names == ""]
    }
  }
  else {
  ...
  for (i in seq_along(arg.names)) {
    if (arg.dim[along, i] > 0) {
      ...
      if (length(dnm.along) == arg.dim[along, i]) {
        ...
        else dnm.along <- paste(arg.names[i], seq_len(arg.dim[along, i]), sep = "")
      }
