...
Some nice properties of these methods include, but are not limited to, the following:
  ...
  1) The `[` and `[<-` operators use different rule-sets for different data.frame-like types (data.frames, tibbles, data.tables, tidytables, etc.). The 'squarebrackets' methods use the same rule-sets for the different data.frame-like types.
  ...
  6) 'squarebrackets' supports index-less sub-set operations, which is faster and more memory efficient than sub-set operations using the `[` and `[<-` operators.
...
## Changelog (EXPERIMENTAL VERSIONS)
 ...
 - 19 October 2024: Removed the renaming methods (`sb_setRename`), and `seq_rec2()`. Added `slcseq()`
