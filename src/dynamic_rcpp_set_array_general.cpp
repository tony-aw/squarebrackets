

#include <Rcpp.h>

#include "squarebrackets.h"

using namespace Rcpp;







inline void rcpp_set_array_general_Logical(
  SEXP x,
  const SEXP s, const SEXP xdims,
  const SEXP rp
) {

  int k;
  
  const void *vmaxsave = vmaxget(); // Because I'm gonna use R_alloc()
  
 // s is a list
 
  k = Rf_length(xdims);

  // allocate vectors (kinda like a list) through R_alloc(); k = ndims(x)
  int **subs = (int**)R_alloc(k, sizeof(int*));
  int *indx = (int*)R_alloc(k, sizeof(int));
  int *bound = (int*)R_alloc(k, sizeof(int));
  R_xlen_t *offset = (R_xlen_t*)R_alloc(k, sizeof(R_xlen_t));
  
  
  // prep
  R_xlen_t n = 1;
  SEXP r;
  
  for (int i = 0; i < k; i++) {
    r = VECTOR_ELT(s, i); 
  	indx[i] = 0;
  	bound[i] = Rf_length(r);
    n *= bound[i];
  	subs[i] = INTEGER(r);
  }
  
  offset[0] = 1;
  for (int i = 1; i < k; i++) {
    offset[i] = offset[i - 1] * INTEGER(xdims)[i - 1];
  }
  
   int *px = LOGICAL(x);
  const int *prp = LOGICAL_RO(rp);
  
  
  // main function
  if(Rf_xlength(rp) == n) {
    R_xlen_t counter = 0;

    for (R_xlen_t i = 0; i < n; i++) {
      R_xlen_t flatind = 0;
    	for (int j = 0; j < k; j++) {
  	    int jj = subs[j][indx[j]];
  	    flatind += (jj - 1) * offset[j];
    	}
    	
    	MACRO_SET_ATOMIC(px, flatind, prp[counter]); // meat in the voodoo sandwich
    	counter++;
    	
    	if (n > 1) {
    	    int j = 0;
    	    while (++indx[j] >= bound[j]) {
        		indx[j] = 0;
        		j = (j + 1) % k;
    	    }
    	}
    }
  }
  else if(Rf_xlength(rp) == 1) {

    for (R_xlen_t i = 0; i < n; i++) {
      R_xlen_t flatind = 0;
    	for (int j = 0; j < k; j++) {
  	    int jj = subs[j][indx[j]];
  	    flatind += (jj - 1) * offset[j];
    	}
    	
    	MACRO_SET_ATOMIC(px, flatind, prp[0]); // meat in the voodoo sandwich
    	
    	if (n > 1) {
    	    int j = 0;
    	    while (++indx[j] >= bound[j]) {
        		indx[j] = 0;
        		j = (j + 1) % k;
    	    }
    	}
    }
  }
  else {
    stop("recycling not allowed");

  }
  

  // Free temporary memory
  vmaxset(vmaxsave);
}






inline void rcpp_set_array_general_Integer(
  SEXP x,
  const SEXP s, const SEXP xdims,
  const SEXP rp
) {

  int k;
  
  const void *vmaxsave = vmaxget(); // Because I'm gonna use R_alloc()
  
 // s is a list
 
  k = Rf_length(xdims);

  // allocate vectors (kinda like a list) through R_alloc(); k = ndims(x)
  int **subs = (int**)R_alloc(k, sizeof(int*));
  int *indx = (int*)R_alloc(k, sizeof(int));
  int *bound = (int*)R_alloc(k, sizeof(int));
  R_xlen_t *offset = (R_xlen_t*)R_alloc(k, sizeof(R_xlen_t));
  
  
  // prep
  R_xlen_t n = 1;
  SEXP r;
  
  for (int i = 0; i < k; i++) {
    r = VECTOR_ELT(s, i); 
  	indx[i] = 0;
  	bound[i] = Rf_length(r);
    n *= bound[i];
  	subs[i] = INTEGER(r);
  }
  
  offset[0] = 1;
  for (int i = 1; i < k; i++) {
    offset[i] = offset[i - 1] * INTEGER(xdims)[i - 1];
  }
  
   int *px = INTEGER(x);
  const int *prp = INTEGER_RO(rp);
  
  
  // main function
  if(Rf_xlength(rp) == n) {
    R_xlen_t counter = 0;

    for (R_xlen_t i = 0; i < n; i++) {
      R_xlen_t flatind = 0;
    	for (int j = 0; j < k; j++) {
  	    int jj = subs[j][indx[j]];
  	    flatind += (jj - 1) * offset[j];
    	}
    	
    	MACRO_SET_ATOMIC(px, flatind, prp[counter]); // meat in the voodoo sandwich
    	counter++;
    	
    	if (n > 1) {
    	    int j = 0;
    	    while (++indx[j] >= bound[j]) {
        		indx[j] = 0;
        		j = (j + 1) % k;
    	    }
    	}
    }
  }
  else if(Rf_xlength(rp) == 1) {

    for (R_xlen_t i = 0; i < n; i++) {
      R_xlen_t flatind = 0;
    	for (int j = 0; j < k; j++) {
  	    int jj = subs[j][indx[j]];
  	    flatind += (jj - 1) * offset[j];
    	}
    	
    	MACRO_SET_ATOMIC(px, flatind, prp[0]); // meat in the voodoo sandwich
    	
    	if (n > 1) {
    	    int j = 0;
    	    while (++indx[j] >= bound[j]) {
        		indx[j] = 0;
        		j = (j + 1) % k;
    	    }
    	}
    }
  }
  else {
    stop("recycling not allowed");

  }
  

  // Free temporary memory
  vmaxset(vmaxsave);
}






inline void rcpp_set_array_general_Numeric(
  SEXP x,
  const SEXP s, const SEXP xdims,
  const SEXP rp
) {

  int k;
  
  const void *vmaxsave = vmaxget(); // Because I'm gonna use R_alloc()
  
 // s is a list
 
  k = Rf_length(xdims);

  // allocate vectors (kinda like a list) through R_alloc(); k = ndims(x)
  int **subs = (int**)R_alloc(k, sizeof(int*));
  int *indx = (int*)R_alloc(k, sizeof(int));
  int *bound = (int*)R_alloc(k, sizeof(int));
  R_xlen_t *offset = (R_xlen_t*)R_alloc(k, sizeof(R_xlen_t));
  
  
  // prep
  R_xlen_t n = 1;
  SEXP r;
  
  for (int i = 0; i < k; i++) {
    r = VECTOR_ELT(s, i); 
  	indx[i] = 0;
  	bound[i] = Rf_length(r);
    n *= bound[i];
  	subs[i] = INTEGER(r);
  }
  
  offset[0] = 1;
  for (int i = 1; i < k; i++) {
    offset[i] = offset[i - 1] * INTEGER(xdims)[i - 1];
  }
  
   double *px = REAL(x);
  const double *prp = REAL_RO(rp);
  
  
  // main function
  if(Rf_xlength(rp) == n) {
    R_xlen_t counter = 0;

    for (R_xlen_t i = 0; i < n; i++) {
      R_xlen_t flatind = 0;
    	for (int j = 0; j < k; j++) {
  	    int jj = subs[j][indx[j]];
  	    flatind += (jj - 1) * offset[j];
    	}
    	
    	MACRO_SET_ATOMIC(px, flatind, prp[counter]); // meat in the voodoo sandwich
    	counter++;
    	
    	if (n > 1) {
    	    int j = 0;
    	    while (++indx[j] >= bound[j]) {
        		indx[j] = 0;
        		j = (j + 1) % k;
    	    }
    	}
    }
  }
  else if(Rf_xlength(rp) == 1) {

    for (R_xlen_t i = 0; i < n; i++) {
      R_xlen_t flatind = 0;
    	for (int j = 0; j < k; j++) {
  	    int jj = subs[j][indx[j]];
  	    flatind += (jj - 1) * offset[j];
    	}
    	
    	MACRO_SET_ATOMIC(px, flatind, prp[0]); // meat in the voodoo sandwich
    	
    	if (n > 1) {
    	    int j = 0;
    	    while (++indx[j] >= bound[j]) {
        		indx[j] = 0;
        		j = (j + 1) % k;
    	    }
    	}
    }
  }
  else {
    stop("recycling not allowed");

  }
  

  // Free temporary memory
  vmaxset(vmaxsave);
}






inline void rcpp_set_array_general_Complex(
  SEXP x,
  const SEXP s, const SEXP xdims,
  const SEXP rp
) {

  int k;
  
  const void *vmaxsave = vmaxget(); // Because I'm gonna use R_alloc()
  
 // s is a list
 
  k = Rf_length(xdims);

  // allocate vectors (kinda like a list) through R_alloc(); k = ndims(x)
  int **subs = (int**)R_alloc(k, sizeof(int*));
  int *indx = (int*)R_alloc(k, sizeof(int));
  int *bound = (int*)R_alloc(k, sizeof(int));
  R_xlen_t *offset = (R_xlen_t*)R_alloc(k, sizeof(R_xlen_t));
  
  
  // prep
  R_xlen_t n = 1;
  SEXP r;
  
  for (int i = 0; i < k; i++) {
    r = VECTOR_ELT(s, i); 
  	indx[i] = 0;
  	bound[i] = Rf_length(r);
    n *= bound[i];
  	subs[i] = INTEGER(r);
  }
  
  offset[0] = 1;
  for (int i = 1; i < k; i++) {
    offset[i] = offset[i - 1] * INTEGER(xdims)[i - 1];
  }
  
   Rcomplex *px = COMPLEX(x);
  const Rcomplex *prp = COMPLEX_RO(rp);
  
  
  // main function
  if(Rf_xlength(rp) == n) {
    R_xlen_t counter = 0;

    for (R_xlen_t i = 0; i < n; i++) {
      R_xlen_t flatind = 0;
    	for (int j = 0; j < k; j++) {
  	    int jj = subs[j][indx[j]];
  	    flatind += (jj - 1) * offset[j];
    	}
    	
    	MACRO_SET_ATOMIC(px, flatind, prp[counter]); // meat in the voodoo sandwich
    	counter++;
    	
    	if (n > 1) {
    	    int j = 0;
    	    while (++indx[j] >= bound[j]) {
        		indx[j] = 0;
        		j = (j + 1) % k;
    	    }
    	}
    }
  }
  else if(Rf_xlength(rp) == 1) {

    for (R_xlen_t i = 0; i < n; i++) {
      R_xlen_t flatind = 0;
    	for (int j = 0; j < k; j++) {
  	    int jj = subs[j][indx[j]];
  	    flatind += (jj - 1) * offset[j];
    	}
    	
    	MACRO_SET_ATOMIC(px, flatind, prp[0]); // meat in the voodoo sandwich
    	
    	if (n > 1) {
    	    int j = 0;
    	    while (++indx[j] >= bound[j]) {
        		indx[j] = 0;
        		j = (j + 1) % k;
    	    }
    	}
    }
  }
  else {
    stop("recycling not allowed");

  }
  

  // Free temporary memory
  vmaxset(vmaxsave);
}






inline void rcpp_set_array_general_Raw(
  SEXP x,
  const SEXP s, const SEXP xdims,
  const SEXP rp
) {

  int k;
  
  const void *vmaxsave = vmaxget(); // Because I'm gonna use R_alloc()
  
 // s is a list
 
  k = Rf_length(xdims);

  // allocate vectors (kinda like a list) through R_alloc(); k = ndims(x)
  int **subs = (int**)R_alloc(k, sizeof(int*));
  int *indx = (int*)R_alloc(k, sizeof(int));
  int *bound = (int*)R_alloc(k, sizeof(int));
  R_xlen_t *offset = (R_xlen_t*)R_alloc(k, sizeof(R_xlen_t));
  
  
  // prep
  R_xlen_t n = 1;
  SEXP r;
  
  for (int i = 0; i < k; i++) {
    r = VECTOR_ELT(s, i); 
  	indx[i] = 0;
  	bound[i] = Rf_length(r);
    n *= bound[i];
  	subs[i] = INTEGER(r);
  }
  
  offset[0] = 1;
  for (int i = 1; i < k; i++) {
    offset[i] = offset[i - 1] * INTEGER(xdims)[i - 1];
  }
  
   Rbyte *px = RAW(x);
  const Rbyte *prp = RAW_RO(rp);
  
  
  // main function
  if(Rf_xlength(rp) == n) {
    R_xlen_t counter = 0;

    for (R_xlen_t i = 0; i < n; i++) {
      R_xlen_t flatind = 0;
    	for (int j = 0; j < k; j++) {
  	    int jj = subs[j][indx[j]];
  	    flatind += (jj - 1) * offset[j];
    	}
    	
    	MACRO_SET_ATOMIC(px, flatind, prp[counter]); // meat in the voodoo sandwich
    	counter++;
    	
    	if (n > 1) {
    	    int j = 0;
    	    while (++indx[j] >= bound[j]) {
        		indx[j] = 0;
        		j = (j + 1) % k;
    	    }
    	}
    }
  }
  else if(Rf_xlength(rp) == 1) {

    for (R_xlen_t i = 0; i < n; i++) {
      R_xlen_t flatind = 0;
    	for (int j = 0; j < k; j++) {
  	    int jj = subs[j][indx[j]];
  	    flatind += (jj - 1) * offset[j];
    	}
    	
    	MACRO_SET_ATOMIC(px, flatind, prp[0]); // meat in the voodoo sandwich
    	
    	if (n > 1) {
    	    int j = 0;
    	    while (++indx[j] >= bound[j]) {
        		indx[j] = 0;
        		j = (j + 1) % k;
    	    }
    	}
    }
  }
  else {
    stop("recycling not allowed");

  }
  

  // Free temporary memory
  vmaxset(vmaxsave);
}






inline void rcpp_set_array_general_Character(
  SEXP x,
  const SEXP s, const SEXP xdims,
  const SEXP rp
) {

  int k;
  
  const void *vmaxsave = vmaxget(); // Because I'm gonna use R_alloc()
  
 // s is a list
 
  k = Rf_length(xdims);

  // allocate vectors (kinda like a list) through R_alloc(); k = ndims(x)
  int **subs = (int**)R_alloc(k, sizeof(int*));
  int *indx = (int*)R_alloc(k, sizeof(int));
  int *bound = (int*)R_alloc(k, sizeof(int));
  R_xlen_t *offset = (R_xlen_t*)R_alloc(k, sizeof(R_xlen_t));
  
  
  // prep
  R_xlen_t n = 1;
  SEXP r;
  
  for (int i = 0; i < k; i++) {
    r = VECTOR_ELT(s, i); 
  	indx[i] = 0;
  	bound[i] = Rf_length(r);
    n *= bound[i];
  	subs[i] = INTEGER(r);
  }
  
  offset[0] = 1;
  for (int i = 1; i < k; i++) {
    offset[i] = offset[i - 1] * INTEGER(xdims)[i - 1];
  }
  
  // SEXP *px = STRING_PTR(x);
  const SEXP *prp = STRING_PTR_RO(rp);
  
  
  // main function
  if(Rf_xlength(rp) == n) {
    R_xlen_t counter = 0;

    for (R_xlen_t i = 0; i < n; i++) {
      R_xlen_t flatind = 0;
    	for (int j = 0; j < k; j++) {
  	    int jj = subs[j][indx[j]];
  	    flatind += (jj - 1) * offset[j];
    	}
    	
    	SET_STRING_ELT(x, flatind, prp[counter]); // meat in the voodoo sandwich
    	counter++;
    	
    	if (n > 1) {
    	    int j = 0;
    	    while (++indx[j] >= bound[j]) {
        		indx[j] = 0;
        		j = (j + 1) % k;
    	    }
    	}
    }
  }
  else if(Rf_xlength(rp) == 1) {

    for (R_xlen_t i = 0; i < n; i++) {
      R_xlen_t flatind = 0;
    	for (int j = 0; j < k; j++) {
  	    int jj = subs[j][indx[j]];
  	    flatind += (jj - 1) * offset[j];
    	}
    	
    	SET_STRING_ELT(x, flatind, prp[0]); // meat in the voodoo sandwich
    	
    	if (n > 1) {
    	    int j = 0;
    	    while (++indx[j] >= bound[j]) {
        		indx[j] = 0;
        		j = (j + 1) % k;
    	    }
    	}
    }
  }
  else {
    stop("recycling not allowed");

  }
  

  // Free temporary memory
  vmaxset(vmaxsave);
}







//' @keywords internal
//' @noRd
// [[Rcpp::export(.rcpp_set_array_general_atomic)]]
void rcpp_set_array_general_atomic(
  SEXP x, SEXP s, SEXP xdims, SEXP rp
) {

    switch(TYPEOF(x)){
    
      case LGLSXP:
      {
         rcpp_set_array_general_Logical(x, s, xdims, rp);
        break;
      }
      case INTSXP:
      {
         rcpp_set_array_general_Integer(x, s, xdims, rp);
        break;
      }
      case REALSXP:
      {
         rcpp_set_array_general_Numeric(x, s, xdims, rp);
        break;
      }
      case CPLXSXP:
      {
         rcpp_set_array_general_Complex(x, s, xdims, rp);
        break;
      }
      case RAWSXP:
      {
         rcpp_set_array_general_Raw(x, s, xdims, rp);
        break;
      }
      case STRSXP:
      {
         rcpp_set_array_general_Character(x, s, xdims, rp);
        break;
      }
      default: stop("unsupported type given");
    }
    
  

}



